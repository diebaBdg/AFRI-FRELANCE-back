...
import AfriFreelance.API.business.user.mappers.UserMapper;
import AfriFreelance.API.config.exceptions.BusinessException;
import AfriFreelance.API.config.exceptions.DuplicateResourceException;
...
public class UserServiceImpl implements IUserService {
    ...
    public UserDTO updateUser(UUID id, UserUpdateDTO updateDTO) {
        ...
        if (updateDTO.getPassword() != null && !updateDTO.getPassword().isEmpty()) {
            if (!PasswordPolicy.isStrong(updateDTO.getPassword())) {
        ...
        if (updateDTO.getRoleCodes() != null && !updateDTO.getRoleCodes().isEmpty()) {
            user.getRoles().clear();
            user.getRoles().addAll(assignRoles(updateDTO.getRoleCodes()));
        }
    ...
    private void validateUserCreation(UserCreateDTO createDTO) {
        ...
        if (!PasswordPolicy.isStrong(createDTO.getPassword())) {
            throw new BusinessException("WEAK_PASSWORD", PasswordPolicy.MESSAGE, null);
        }
